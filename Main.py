...
class MainWindow(QTabWidget):
    ...
    def __init__(self):
        # 初始化界面
        super().__init__()
        self.setWindowTitle('基于YOLO V5火灾预警检测')
    ...
    ***界面初始化***
    ...
    def initUI(self):
        ...
        img_detection_title = QLabel("Image_detect")
        ...
        vid_title = QLabel("Video_detect")
        vid_title.setFont(font_title)
        self.vid_img = QLabel()
        self.vid_img.setPixmap(QPixmap("images/UI/up.jpeg"))
        vid_title.setAlignment(Qt.AlignCenter)
        self.vid_img.setAlignment(Qt.AlignCenter)
        self.webcam_detection_btn = QPushButton("实时监测")
        ...
        self.left_img.setAlignment(Qt.AlignCenter)
        self.addTab(img_detection_widget, 'Image')
    ...
    ***上传图片***
    ...
    def upload_img(self):
        ...
        if fileName:
            ...
            if im0 is not None:
                resize_scale = self.output_size / max(im0.shape[:2])
                im0_resized = cv2.resize(im0, None, fx=resize_scale, fy=resize_scale)

                # 保存调整大小后的图片
                save_result_path = os.path.join("images", "tmp", "upload_show_result.jpg")
                cv2.imwrite(save_result_path, im0_resized)

                # 在界面上显示上传的图片
                self.left_img.setPixmap(QPixmap(save_result_path))
                self.img2predict = save_path
                self.right_img.setPixmap(QPixmap("images/UI/right.jpeg"))
            else:
                QMessageBox.warning(self, "Error", "Failed to read the image file.")
    ...
    ***检测图片***
        ...
        if source == "":
            ...
            for path, im, im0s, vid_cap, s in dataset:
                ...
                for i, det in enumerate(pred):  # per image
                        ...
                        for *xyxy, conf, cls in reversed(det):
                            if save_txt:  # Write to file
                                ...
                                if int(cls) == 0:
                                    ...
                                    with open("fire_detect.txt", "a") as file:
                                        file.write(f"{current_time}: 警告，检测到火灾！！\n")
                                    # 创建警告弹窗
                                    msg_box = QMessageBox()
                                    msg_box.setWindowTitle("火灾预警检测")
                                    msg_box.setText(f"{current_time}: 警告，检测到火灾，记录已保存至当前同目录 fire_detet.txt 中！！")
                                    ...
                                    # 设置两秒后关闭弹窗
                                    QTimer.singleShot(2000, close_message_box)
                    ...
                    # Currently, there might be issues only on Ubuntu, but continue
    ...
    def detect_vid(self):
        ...
        # Define the path for saving the fire detection log file
        log_file_path = "fire_detect_video.txt"
        ...
        for path, im, im0s, vid_cap, s in dataset:
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    fire_detected = False
                    for *xyxy, conf, cls in reversed(det):
                        if int(cls) == 0:  # Assuming fire class index is 0
                            fire_detected = True
                            break
                    ...
                    if fire_detected:
                        ...
                        print(f"{current_time}: 警告，检测到火灾！！")
                        # Write to log file
                        with open("fire_video_detect.txt", "a") as file:
                            file.write(f"{current_time}: 警告，检测到火灾！！\n")
                ...
                # Break the loop if stop event is set
                if cv2.waitKey(25) & self.stopEvent.is_set():
                    self.stopEvent.clear()
